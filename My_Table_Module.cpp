...
int Keys(table<T1>&b){
	...
	if(b.body==NULL)out=EMPTY_TABLE;
		...
		int local=0;
		int *key=new int[b.col];
		for(unsigned i=0;i<b.col;i++)key[i]=0;
		for(unsigned i=0;i<b.row;i++){
			for(unsigned j=0;j<b.col;j++){
                if(b.body[i][j]<0)key[j]+=b.body[i][j];
			}
		}
		for(unsigned i=0;i<b.col;i++)cout<<key[i]<<" ";
		out=Quick_Sort(b,key,0,b.col);
		delete[]key;
	}
	return out;
}
        ...
        if(from >= to){
            ...
            while(i <= j){
                    ...
                    if(i <= j){
                        ...
                        //b.body[i]=b.body[j];
                        //b.body[j]=tmp;
                        for(unsigned i1=0;i1<b.row;i1++){
                            tmp=b.body[i1][i];
                            b.body[i1][i]=b.body[i1][j];
                            b.body[i1][j]=tmp;
                        }
                        i++;
                        j--;
